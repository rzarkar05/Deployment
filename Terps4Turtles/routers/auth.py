...
templates = Jinja2Templates(directory="Terps4Turtles/templates")
...
def authenticate_user(username: str, password: str, db):
    user = db.query(Terps4Turtles.models.Users)\
        .filter(Terps4Turtles.models.Users.username == username)\
        .first()
...
async def login(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        response = RedirectResponse(url="/terps4turtles/map", status_code=status.HTTP_302_FOUND)
...
async def register_user(request: Request, email: str = Form(...), username: str = Form(...), 
    ...
    validation1 = db.query(Terps4Turtles.models.Users).filter(Terps4Turtles.models.Users.username == username).first()
    validation2 = db.query(Terps4Turtles.models.Users).filter(Terps4Turtles.models.Users.email == email).first()
    if password != password2 or validation1 is not None or validation2 is not None:
        msg = "Invalid registration request"
        return templates.TemplateResponse("register.html", {"request": request, "msg": msg})
    user_model = Terps4Turtles.models.Users()
    ...
    return RedirectResponse(url = '/terps4turtles/login')
