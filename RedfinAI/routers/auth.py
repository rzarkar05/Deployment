...
def authenticate_user(username: str, password: str, db):
    user = db.query(RedfinAI.models.Users)\
        .filter(RedfinAI.models.Users.username == username)\
        .first()
...
async def login(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        response = RedirectResponse(url="/RedfinAI/filters/", status_code=status.HTTP_302_FOUND)
...
async def register_user(request: Request, email: str = Form(...), username: str = Form(...), 
    ...
    validation1 = db.query(RedfinAI.models.Users).filter(RedfinAI.models.Users.username == username).first()
    validation2 = db.query(RedfinAI.models.Users).filter(RedfinAI.models.Users.email == email).first()
    if password != password2 or validation1 is not None or validation2 is not None:
        msg = "Invalid registration request"
        return templates.TemplateResponse("index.html", {"request": request, "msg": msg})
    user_model = RedfinAI.models.Users()
    user_model.username = username
    user_model.email = email
    user_model.hashed_pass = get_password_hash(password)
    user_model.saved = []
    user_model.refresh_time = ""
    db.add(user_model)
    db.commit()
    msg = "User successfully created"
    return RedirectResponse(url = '/RedfinAI/auth/login')