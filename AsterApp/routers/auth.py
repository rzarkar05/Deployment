...
#Instead of making a whole separate app, routes to the existing one in main.py
router = APIRouter(
    prefix = '/aster-app/auth',
    tags=['auth']
)
...
def authenticate_user(username: str, password: str, db):
    user = db.query(AsterApp.models.Users)\
        .filter(AsterApp.models.Users.username == username)\
        .first()
...
async def login(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        response = RedirectResponse(url="/aster-app/todos", status_code=status.HTTP_302_FOUND)
...
async def register_user(request: Request, email: str = Form(...), username: str = Form(...), 
    ...
    validation1 = db.query(AsterApp.models.Users).filter(AsterApp.models.Users.username == username).first()
    validation2 = db.query(AsterApp.models.Users).filter(AsterApp.models.Users.email == email).first()
    if password != password2 or validation1 is not None or validation2 is not None:
        msg = "Invalid registration request"
        return templates.TemplateResponse("register.html", {"request": request, "msg": msg})
    user_model = AsterApp.models.Users()
